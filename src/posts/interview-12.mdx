...
slug: interview-preparation-algorithm-running-time
...
Welcome to part-12 of the series and the part on Algorithm running time questions. A lot of time, the interviewer will ask you to write an algorithm and then ask you the Big O or worst running time of it. This part will teach you how to deal with questions like them.
...
**Answer-** The order of Big O is `O(1) < O(log n) < O(n) < O(n log n) < O(n²)`
