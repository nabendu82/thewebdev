...
slug: interview-preparation-tricky-questions-2
...
```
    console.log([] + []);
```
**Answer-**The output logged will be: `“ “` ie two empty spaces.
...
The reason is the the line **const sentence = a `hi`;** also passes an argument to the function and it is know as tagged template. It is quite similar to the line **const sentence = a(`hi`);**
...
```
    ...
    console.log(x);
```
**Answer-**The output will be `1,2,34,5,6`
...
**Answer-**In JavaScript when the parser reaches the `<script>` tag, it stops to fetch it(if external file) and run it.

This can be a problem, because it generally stops our html code been rendered to the DOM and visible to the user. One way to solve this issue is to put the `<script>` tag just before the body tage ends ie `</body>`. But in some cases we might require some features from our javascript to be available to the page.
...
You can go through my earlier blog [here](https://thewebdev.tech/ydkjs-scopes-and-closures-3) to understand how the compiler and interpreter step runs.
